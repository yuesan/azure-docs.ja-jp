...
シミュレートされたデバイスによってメッセージに添付されたプロパティに基づいて、メッセージを異なるリソースにルーティングします。またカスタム ルーティングされないメッセージは、既定のエンドポイント (メッセージ/イベント) に送信されます。 
次のチュートリアルでは、メッセージを IoT Hub に送信して、それらがさまざまな宛先にルーティングされるのを確認します。
